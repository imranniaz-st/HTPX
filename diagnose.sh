...
docker logs server-manager-app 2>/dev/null | tail -20 || echo "⚠️  No logs or container not running"
echo ""

echo "🔍 Recent Logs (nginx container):"
docker logs server-manager-nginx 2>/dev/null | tail -20 || echo "⚠️  No logs or container not running"
echo ""

echo "💾 Database Status:"
docker logs server-manager-mysql 2>/dev/null | tail -10 || echo "⚠️  MySQL not running"
echo ""
