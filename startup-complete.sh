...
if [ ! -d "frontend/dist" ] || [ -z "$(ls -A frontend/dist 2>/dev/null)" ]; then
    echo "⚠️  frontend/dist is empty or missing"
    ...
    if [ -d "frontend" ]; then
        ...
        echo "⚠️  Frontend directory not found - will create dummy dist folder"
...
if curl -s -I http://localhost:8001/ | grep -q "200\|301\|302"; then
    ...
    echo "⚠️  Getting initial response, might be warming up..."
